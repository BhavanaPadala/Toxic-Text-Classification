...
X_train, X_test, y_train, y_test = train_test_split(train['comment_text'], train['severe_toxic'], test_size=0.2, random_state=42)
...
bud['severe_toxic'] = y_pred_x
y_prob = lr.predict_proba(test_dtm)[:,1]
sub['severe_toxic']= y_prob
...
with open('severe_toxic_model.pkl','wb') as f:
    pickle.dump(lr,f)
