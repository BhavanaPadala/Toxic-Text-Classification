...
X_train, X_test, y_train, y_test = train_test_split(train['comment_text'], train['identity_hate'], test_size=0.2, random_state=42)
...
bud['identity_hate'] = y_pred_x
y_prob = lr.predict_proba(test_dtm)[:,1]
sub['identity_hate']= y_prob
...
with open('identity_hate_model.pkl','wb') as f:
    pickle.dump(lr,f)
